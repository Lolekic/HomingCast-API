...
`Custom Trajectory` is a property called `Trajectory`, which makes the projectile follow a predefined path
...
!!! question "What is a `Checkpoint`?"
    A checkpoint contains a name. When the projectile reaches this point, it will fire the `CheckpointReached` callback.
